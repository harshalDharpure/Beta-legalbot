...
def main() -> None:
    ...
    ap.add_argument("--config", default="research_pipeline/configs/pipeline_default.yaml")
    ap.add_argument("--train-jsonl", default="research_pipeline/data/splits/train.jsonl")
    ap.add_argument("--init-from", choices=["base", "exp3"], default="base")
    ap.add_argument("--out-root", default="research_pipeline/checkpoints/stage2_ablations")
    args = ap.parse_args()
    ...
    for m in modes:
        ...
        cmd = [
            sys.executable,
            str(_REPO / "research_pipeline/stage2_multi_objective/train.py"),
            "--config",
